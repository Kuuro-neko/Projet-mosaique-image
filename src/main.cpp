...
void fonctionButtonCreerImage(Fl_Widget* widget, void* data) {
    ...
    cv::Mat mosaic;
    if(A){
        mosaic=generateMosaicUsingAlignment(inputImage, param->blockSize, param->datasetPath, params);
    }else{
        mosaic=generateMosaic(inputImage, meanValues, param->blockSize, params);
    }
...
void fonctionChoisirImage(Fl_Widget* widget, void* data) {
    ...
    Fl_File_Chooser *fileChooser=new Fl_File_Chooser("/home/thibaut/Downloads/projet_image/Projet-mosaique-image/img", NULL, Fl_File_Chooser::SINGLE, "images");
    fileChooser->show();
